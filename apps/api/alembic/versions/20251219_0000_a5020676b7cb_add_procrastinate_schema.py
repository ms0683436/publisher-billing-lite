...
Revision ID: a5020676b7cb
Revises: 7916de66bc80
Create Date: 2025-12-19 00:00:04.457238+08:00

Schema SQL generated by: procrastinate schema --read
"""
...
revision: str = "a5020676b7cb"
down_revision: str | Sequence[str] | None = "7916de66bc80"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

# Procrastinate 3.x schema SQL (generated by `procrastinate schema --read`)
...
SCHEMA_STATEMENTS = [
    ...
    # Functions
    """
    CREATE FUNCTION procrastinate_defer_jobs_v1(jobs procrastinate_job_to_defer_v1[])
        RETURNS bigint[]
        LANGUAGE plpgsql
    AS $$
    DECLARE
    ...
    BEGIN
        WITH inserted_jobs AS (
            ...
            SELECT (job).queue_name, (job).task_name, (job).priority, (job).lock, (job).queueing_lock, (job).args, (job).scheduled_at
    ...
    BEGIN
            ...
            SET job_id = (
                SELECT COALESCE((
                    SELECT unnest(procrastinate_defer_jobs_v1(
                        ARRAY[ROW(_queue_name, _task_name, _priority, _lock, _queueing_lock, _args, NULL::timestamptz)]::procrastinate_job_to_defer_v1[]
                    ))
                ), NULL)
            )
            WHERE id = _defer_id
        ...
        DELETE FROM procrastinate_periodic_defers
            USING (
                SELECT id FROM procrastinate_periodic_defers
                WHERE task_name = _task_name AND periodic_id = _periodic_id AND defer_timestamp < _defer_timestamp
    ...
    """,
    """
    CREATE FUNCTION procrastinate_fetch_job_v2(target_queue_names character varying[], p_worker_id bigint)
    ...
    BEGIN
        WITH candidate AS (
            SELECT jobs.*
                ...
                WHERE
                    NOT EXISTS (
                        SELECT 1 FROM procrastinate_jobs AS other_jobs
                        WHERE jobs.lock IS NOT NULL AND other_jobs.lock = jobs.lock
                            AND (other_jobs.status = 'doing'
                                OR (other_jobs.status = 'todo'
                                    AND (other_jobs.priority > jobs.priority
    ...
    BEGIN
        ...
        END IF;
        IF delete_job THEN
            DELETE FROM procrastinate_jobs WHERE id = job_id AND status IN ('todo', 'doing') RETURNING id INTO _job_id;
        ELSE
            UPDATE procrastinate_jobs
                SET status = end_status, abort_requested = false,
                    attempts = CASE status WHEN 'doing' THEN attempts + 1 ELSE attempts END
    ...
    BEGIN
        IF delete_job THEN
            DELETE FROM procrastinate_jobs WHERE id = job_id AND status = 'todo' RETURNING id INTO _job_id;
        END IF;
        IF _job_id IS NULL THEN
            IF abort THEN
                UPDATE procrastinate_jobs
                    SET abort_requested = true,
                        status = CASE status WHEN 'todo' THEN 'cancelled'::procrastinate_job_status ELSE status END
                    WHERE id = job_id AND status IN ('todo', 'doing')
                    RETURNING id INTO _job_id;
            ELSE
                UPDATE procrastinate_jobs SET status = 'cancelled'::procrastinate_job_status
                    WHERE id = job_id AND status = 'todo' RETURNING id INTO _job_id;
            END IF;
        END IF;
        RETURN _job_id;
    END;
    ...
    CREATE FUNCTION procrastinate_retry_job_v1(
        job_id bigint, retry_at timestamp with time zone, new_priority integer,
        new_queue_name character varying, new_lock character varying
    ) RETURNS void LANGUAGE plpgsql AS $$
    DECLARE
        _job_id bigint;
        _abort_requested boolean;
    BEGIN
        SELECT abort_requested FROM procrastinate_jobs WHERE id = job_id AND status = 'doing' FOR UPDATE INTO _abort_requested;
        IF _abort_requested THEN
            UPDATE procrastinate_jobs SET status = 'failed'::procrastinate_job_status
                WHERE id = job_id AND status = 'doing' RETURNING id INTO _job_id;
        ELSE
            UPDATE procrastinate_jobs
                SET status = 'todo'::procrastinate_job_status, attempts = attempts + 1, scheduled_at = retry_at,
                    priority = COALESCE(new_priority, priority), queue_name = COALESCE(new_queue_name, queue_name),
    ...
    BEGIN
        SELECT status, abort_requested FROM procrastinate_jobs
            WHERE id = job_id AND status IN ('doing', 'failed') FOR UPDATE INTO _current_status, _abort_requested;
        ...
        ELSE
            UPDATE procrastinate_jobs
                SET status = 'todo'::procrastinate_job_status, attempts = attempts + 1, scheduled_at = retry_at,
                    priority = COALESCE(new_priority, priority), queue_name = COALESCE(new_queue_name, queue_name),
                    lock = COALESCE(new_lock, lock)
                WHERE id = job_id AND status IN ('doing', 'failed') RETURNING id INTO _job_id;
        END IF;
        IF _job_id IS NULL THEN
    ...
    CREATE FUNCTION procrastinate_trigger_function_status_events_insert_v1()
        RETURNS trigger LANGUAGE plpgsql AS $$
    BEGIN
        INSERT INTO procrastinate_events(job_id, type) VALUES (NEW.id, 'deferred'::procrastinate_job_event_type);
        RETURN NEW;
    END;
    $$
    """,
    ...
    CREATE FUNCTION procrastinate_trigger_function_scheduled_events_v1()
        RETURNS trigger LANGUAGE plpgsql AS $$
    BEGIN
        INSERT INTO procrastinate_events(job_id, type, at) VALUES (NEW.id, 'scheduled'::procrastinate_job_event_type, NEW.scheduled_at);
        RETURN NEW;
    END;
    $$
    """,
    ...
    CREATE FUNCTION procrastinate_trigger_abort_requested_events_procedure_v1()
        RETURNS trigger LANGUAGE plpgsql AS $$
    BEGIN
        INSERT INTO procrastinate_events(job_id, type) VALUES (NEW.id, 'abort_requested'::procrastinate_job_event_type);
        RETURN NEW;
    END;
    $$
    """,
    ...
    CREATE FUNCTION procrastinate_unlink_periodic_defers_v1()
        RETURNS trigger LANGUAGE plpgsql AS $$
    BEGIN
        UPDATE procrastinate_periodic_defers SET job_id = NULL WHERE job_id = OLD.id;
        RETURN OLD;
    END;
    $$
    """,
    ...
    CREATE FUNCTION procrastinate_register_worker_v1()
        RETURNS TABLE(worker_id bigint) LANGUAGE plpgsql AS $$
    BEGIN
        RETURN QUERY INSERT INTO procrastinate_workers DEFAULT VALUES RETURNING procrastinate_workers.id;
    END;
    $$
    """,
    """
    ...
    CREATE FUNCTION procrastinate_update_heartbeat_v1(worker_id bigint)
        RETURNS void LANGUAGE plpgsql AS $$
    BEGIN
        UPDATE procrastinate_workers SET last_heartbeat = NOW() WHERE id = worker_id;
    END;
    $$
    """,
    """
    CREATE FUNCTION procrastinate_prune_stalled_workers_v1(seconds_since_heartbeat float)
        RETURNS TABLE(worker_id bigint) LANGUAGE plpgsql AS $$
    BEGIN
        RETURN QUERY DELETE FROM procrastinate_workers
...
def upgrade() -> None:
    """Create Procrastinate schema tables."""
    for stmt in SCHEMA_STATEMENTS:
        op.execute(stmt)


def downgrade() -> None:
    """Drop all Procrastinate schema objects.

    Uses CASCADE on tables to automatically drop dependent triggers and indices.
    This approach is more robust than explicitly dropping triggers, which can
    fail if the table doesn't exist in a partially-applied state.
    """
    # Drop tables first with CASCADE (automatically drops triggers and indices)
    ...
    # Drop functions
    op.execute("DROP FUNCTION IF EXISTS procrastinate_prune_stalled_workers_v1 CASCADE")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_update_heartbeat_v1 CASCADE")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_unregister_worker_v1 CASCADE")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_register_worker_v1 CASCADE")
    op.execute(
    ...
    op.execute("DROP FUNCTION IF EXISTS procrastinate_retry_job_v2 CASCADE")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_retry_job_v1 CASCADE")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_cancel_job_v1 CASCADE")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_finish_job_v1 CASCADE")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_fetch_job_v2 CASCADE")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_defer_periodic_job_v2 CASCADE")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_defer_jobs_v1 CASCADE")
